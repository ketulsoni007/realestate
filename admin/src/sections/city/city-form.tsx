...
import { stateDropdownApi, stateStoreApi, stateUpdateApi } from 'src/store/Slices/stateSlice';
import { cityStoreApi, cityUpdateApi } from 'src/store/Slices/citySlice';

const CityForm = ({ currentModule }:any) => {
  const dispatch = useAppDispatch();
  const navigate = useNavigate();
  const isStateDropDownOption = useSelector((state:any)=>state.state.isStateDropDownOption);
  const isApiStatus = useSelector((state:any)=>state.state.isApiStatus);
  const dropDownLoading = isApiStatus?.stateDropdownApi === 'loading';

  useEffect(()=>{
    dispatch(stateDropdownApi({}))
  },[]);
  ...
  const handleSubmit = (values:any, { setErrors, setStatus, setSubmitting, resetForm }:any) => {
    try {
      if (values?._id) {
        dispatch(cityUpdateApi(values))
          .then((action) => {
            if (action?.meta?.requestStatus === "fulfilled") {
              navigate('/city');
              toastNotificationAdmin.success('City updated successfully');
        ...
        dispatch(cityStoreApi(values))
          .then((action) => {
            if (action?.meta?.requestStatus === "fulfilled") {
              navigate('/city');
              toastNotificationAdmin.success('City created successfully');
            } else if (action?.meta?.requestStatus === "rejected") {
              const message = action?.payload?.message ?? 'Something went wrong';
              const status = action?.payload?.status ?? 410;
              const errors = action?.payload?.errors ?? [];
              ...
              return (
                <Form onSubmit={handleSubmit}>
                  <Grid container spacing={2}>
                    ...
                    <Grid item xs={12}>
                      <Field name="state">{({ field }:any) => <CustomSelect {...field} label="State" error={touched.state && !!errors.state} helperText={touched.state && errors.state} options={isStateDropDownOption} loading={dropDownLoading} />}</Field>
                    </Grid>
                    <Grid item xs={12}>
