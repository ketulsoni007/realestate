  ...
  return (
    <>
      <TableRow hover tabIndex={-1} role="checkbox" selected={selected}>
        ...
        <TableCell>{row?.state?.country?.name}</TableCell>
        <TableCell>
      ...
      >
        ...
        >
          <MenuItem
            onClick={() => {
              dispatch(cityDetailApi({ _id: row?._id })).then((action) => {
                if (action?.meta?.requestStatus === 'fulfilled') {
                  navigate(`/city/edit/${row?._id}`);
          ...
          <MenuItem
            onClick={() => {
              showConfirmation({
                ...
                if (result) {
                  dispatch(cityDeleteApi({ ids: [row?._id] })).then((action: any) => {
                    if (action?.meta?.requestStatus === 'fulfilled') {
